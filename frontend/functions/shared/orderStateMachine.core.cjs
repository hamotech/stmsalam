/** Cloud Functions / Node canonical. Keep in sync with `shared/orderStateMachine.core.esm.js` (Vite / Expo). */
const VALID_STATES = [
  'pending_payment',
  'placed',
  'paid',
...
function getNextAllowedStates(state, actor) {
...
function assertValidOrderTransition(fromRaw, toRaw, actor) {
...
function assertPaymentConsistency(statusRaw, paymentStatusRaw, opts) {
...
function normalizeLegacyCodBusinessStatusRead(doc, normalizedCanonical) {
  const pm = String(doc?.paymentMethod || '').trim().toUpperCase();
...
function readCanonicalOrderStatusStrict(doc, opts) {
...
function normalizeOrderStateForRead(doc, opts) {
  const logger = opts?.logger || (() => {});
  ...
  const paymentMethod = String(doc?.paymentMethod || '')
    .trim()
    .toUpperCase();
  ...
  const rawPaymentMethod = String(doc?.paymentMethod || '')
    .trim()
    .toUpperCase();
