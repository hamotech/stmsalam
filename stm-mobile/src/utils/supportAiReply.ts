...
/** On-device FAQ assistant — same logic as web `supportAiReply.js`. */
export function getSupportAiReply(userText: string): string {
  ...
  if (/track|order status|where is my|stm-/.test(q)) {
    return `Open **Orders** or **Track** in this app and enter your **order ID** (e.g. STM-…). If you're stuck, use **Live team** in **Help** or WhatsApp **${shopInfo.phone}**.`;
  }

  if (/pay|payment|paynow|qr|stripe|paypal|cash/.test(q)) {
    return `We support **PayNow (SGQR)**, **card checkout**, and **cash** (where available). For PayNow help, use **Help → Live team** or WhatsApp us.`;
  }

  if (/menu|food|halal|vegetarian|spicy|price/.test(q)) {
    return `Browse the **Menu** on the site or ask for today's specials. For ingredient or halal details, tap **Live team** or WhatsApp **${shopInfo.phone}**.`;
  }

  if (/phone|whatsapp|call|contact/.test(q)) {
    return `Reach us at **${shopInfo.phone}** or WhatsApp **${shopInfo.whatsapp}**. For follow-up on an existing order, **Live team** in Help is best.`;
  }

  if (/human|agent|staff|real person|admin/.test(q)) {
    return `Switch to the **Live team** tab in Help — staff can read your thread from the admin dashboard and reply when they're available.`;
  }
  ...
  return (
    ...
    `For anything personal (payments, allergies, special requests), open **Live team** or WhatsApp **${shopInfo.phone}**.\n\n` +
    `_Outlet:_ ${outlet}`
  );
}
